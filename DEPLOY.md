...
scp -i C:\Users\Atoms\.ssh\yieldsoft_hetzner_ed25519 `
  "C:\Users\Atoms\Downloads\itinera-v2.4.0.zip" `
...
```bash
ls -lh /root/itinera-v2.4.0.zip
sha256sum /root/itinera-v2.4.0.zip
```

El valor debe coincidir con `itinera-v2.4.0.zip.sha256`.
...
BACKUP="/opt/backups/itinera-v2/pre-v2.4.0-$(date +%Y%m%d-%H%M%S).sql"
...
```bash
rm -rf /opt/releases/itinera-v2.4.0
mkdir -p /opt/releases/itinera-v2.4.0

unzip -q \
  /root/itinera-v2.4.0.zip \
  -d /opt/releases/itinera-v2.4.0

rsync -a --delete \
  --exclude='.env.production' \
  --exclude='.env.production.backup-*' \
  /opt/releases/itinera-v2.4.0/ \
  /opt/itinera-v2/
...
El endpoint `live` debe indicar `"version":"2.4.0"`; `ready`, Itinera v1 y Yieldsoft deben responder correctamente.

## 8. Actualización de GitHub y Netlify

1. Descomprimir `itinera-v2.4.0.zip` en Windows.
...
```text
v2.4.0: itinerarios por días y diseño minimalista
```
...
- Crear un itinerario indicando entre 1 y 10 días.
- Editar posteriormente la duración desde configuración.
- Confirmar que las cabeceras muestran fechas sin «Día 1», «Día 2», etc.
- Comprobar que los planes aparecen ordenados por hora, sin filas horarias.
- Revisar los tooltips del menú y de las acciones al pasar el ratón.
- Comprobar la vista diaria desde móvil.
